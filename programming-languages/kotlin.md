# Kotlin

## Links

- [JetBrains Academy: Learn Kotlin](https://hyperskill.org/tracks?category=4)
- [A lightweight Kotlin web framework for backend developers](https://github.com/kwebio/kweb-core)

## Frameworks & libraries

- [Kotlin ORM lib](https://www.ktorm.org/)
- [Ktor is a framework to easily build connected applications – web applications, HTTP services, mobile and browser applications.](https://ktor.io/)
- [Koin. The pragmatic Kotlin dependency injection framework](https://insert-koin.io/)
- [Dependency Injection using Koin](https://blogs.halodoc.io/dependency-injection-with-koin/)
- [An opinionated Kotlin backend service - Database / Migration / ORM](https://medium.com/nerd-for-tech/an-opinionated-kotlin-backend-service-database-migration-orm-52527ce3228)
- [How to use an in-memory database for testing on Ktor](https://www.marcogomiero.com/posts/2021/ktor-in-memory-db-testing/)
- [KtorEasy](https://github.com/mathias21/KtorEasy)
- [KotlinConf 2018 - Building Server Backends with Ktor by Ryan Harter]( https://youtu.be/V4PS3IjIzlw)
- [Kotlin linter in spirit of standard/standard (JavaScript) and gofmt (Go)](https://pinterest.github.io/ktlint/)
